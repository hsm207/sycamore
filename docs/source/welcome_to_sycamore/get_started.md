...
Run the Sycamore HTTP Crawler container with an additional parameter: 
...
Sycamore will automatically start processing the new data into the existing index from the previous demo. The processing job is complete and the data is loaded into the index once you see log messages similar to: 
...
Check to see if you are missing environment variables. If so, please set them using the instructions earlier in the guide. 

If it is constantly OOMing, increase the memory for your Docker container 
...
If you see an error message like: opensearch to start did not return true with 300 tries 
